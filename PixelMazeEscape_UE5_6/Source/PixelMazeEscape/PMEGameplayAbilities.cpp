...
bool UPMEGameplayAbility::ConsumeItem(const FGameplayAbilityActorInfo* ActorInfo, const EPMEPickupType ItemType) const
{
    ...
    return PlayerState && PlayerState->GetInventoryComponent() && PlayerState->GetInventoryComponent()->ConsumeItem(ItemType, 1);
}

void UPMEGameplayAbility::NotifySuccessfulItemUse(
    const FGameplayAbilityActorInfo* ActorInfo,
    const EPMEPickupType ItemType) const
{
    APMEPlayerCharacter* Character =
        GetMazeCharacter(ActorInfo);
    if (!Character)
    {
        return;
    }
    ...
    if (APMEPlayerController* Controller =
        Cast<APMEPlayerController>(
            Character->GetController()))
    {
        Controller->ClientNotifyItemUsed(ItemType);
    }
}
    ...
    const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    ...
    {
        SprintEffectHandle = ApplyGameplayEffectToOwner(Handle, ActorInfo, ActivationInfo, UPMEGE_Sprint::StaticClass()->GetDefaultObject<UGameplayEffect>(), 1.0f);
    ...
    const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    ...
    {
        NotifySuccessfulItemUse(
            ActorInfo,
            EPMEPickupType::Torch);
        FGameplayEffectSpecHandle SpecHandle = MakeOutgoingGameplayEffectSpec(UPMEGE_Torch::StaticClass(), 1.0f);
        if (SpecHandle.IsValid())
        {
            const float Duration = BaseDuration + DurationPerUpgradeStack * PlayerState->GetUpgradeStack(EPMEUpgradeType::LongerTorch);
...
void UPMEGA_MapPulse::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
    const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    APMEPlayerState* PlayerState = GetMazePlayerState(ActorInfo);
    ...
    if (Character && ConsumeItem(ActorInfo, EPMEPickupType::MapPulse))
    {
        NotifySuccessfulItemUse(
            ActorInfo,
            EPMEPickupType::MapPulse);
        if (APMEPlayerController* PC = Cast<APMEPlayerController>(Character->GetController()))
        {
            PC->ClientActivateMapPulse(BaseDuration + (PlayerState ? PlayerState->GetUpgradeStack(EPMEUpgradeType::BiggerMapPulse) * DurationPerUpgradeStack : 0.0f));
        }
    }
    EndAbility(Handle, ActorInfo, ActivationInfo, true, false);
}
    ...
    const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    ...
    if (Character && ConsumeItem(ActorInfo, EPMEPickupType::Decoy))
    {
        NotifySuccessfulItemUse(
            ActorInfo,
            EPMEPickupType::Decoy);
        if (APMEGameModeBase* GM = Character->GetWorld()->GetAuthGameMode<APMEGameModeBase>()) GM->SpawnDecoy(Character);
    }
    EndAbility(Handle, ActorInfo, ActivationInfo, true, false);
}

UPMEGA_LightBomb::UPMEGA_LightBomb() {}
void UPMEGA_LightBomb::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
    const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    APMEPlayerCharacter* Character = GetMazeCharacter(ActorInfo);
    if (Character && ConsumeItem(ActorInfo, EPMEPickupType::LightBomb))
    {
        NotifySuccessfulItemUse(
            ActorInfo,
            EPMEPickupType::LightBomb);
        if (APMEGameModeBase* GM = Character->GetWorld()->GetAuthGameMode<APMEGameModeBase>()) GM->StunEnemiesAround(Character->GetActorLocation(), RadiusInTiles, StunDuration);
    }
    EndAbility(Handle, ActorInfo, ActivationInfo, true, false);
}

UPMEGA_MasterKey::UPMEGA_MasterKey() {}
void UPMEGA_MasterKey::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
    const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    APMEPlayerState* PlayerState = GetMazePlayerState(ActorInfo);
    if (PlayerState && ConsumeItem(ActorInfo, EPMEPickupType::MasterKey))
    {
        if (APMEGameModeBase* GM = PlayerState->GetWorld()->GetAuthGameMode<APMEGameModeBase>()) GM->UseMasterKey(PlayerState);
    }
    EndAbility(Handle, ActorInfo, ActivationInfo, true, false);
}
    ...
    const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    ...
    if (Character)
    {
        if (APMEGameModeBase* GM = Character->GetWorld()->GetAuthGameMode<APMEGameModeBase>()) GM->TryReviveNearestTeammate(Character);
    }
    EndAbility(Handle, ActorInfo, ActivationInfo, true, false);
}
