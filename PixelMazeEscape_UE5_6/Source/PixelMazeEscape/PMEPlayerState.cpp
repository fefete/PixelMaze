	...
	if (AbilitySystemComponent)
	{
		AbilitySystemComponent->SetNumericAttributeBase(UPMEAttributeSet::GetHealthAttribute(),
		                                                AbilitySystemComponent->GetNumericAttribute(
			                                                UPMEAttributeSet::GetMaxHealthAttribute()));
...
void APMEPlayerState::SetDowned(const bool bNewDowned)
{
	...
	if (AbilitySystemComponent)
	{
		...
		if (bDowned) AbilitySystemComponent->AddLooseGameplayTag(Tag);
		else AbilitySystemComponent->RemoveLooseGameplayTag(Tag);
	}
	OnRep_Downed();
	ForceNetUpdate();
}

void APMEPlayerState::AddScore(const int32 Amount)
{
	check(HasAuthority());
	RunScore = FMath::Max(0, RunScore + Amount);
	ForceNetUpdate();
}

void APMEPlayerState::RecordDetection()
{
	check(HasAuthority());
	++DetectionCount;
	ForceNetUpdate();
...
void APMEPlayerState::RecordHit()
{
	check(HasAuthority());
	++HitCount;
	ForceNetUpdate();
...
void APMEPlayerState::RecordObjective()
{
	check(HasAuthority());
	++ObjectivesActivated;
	AddScore(250);
...
void APMEPlayerState::AddDistanceTravelled(const float Distance)
{
	if (HasAuthority()) DistanceTravelled += FMath::Max(0.0f, Distance);
}
...
{
	if (APMEPlayerCharacter* C = GetPawn<APMEPlayerCharacter>()) C->RefreshPlayerVisual();
}

void APMEPlayerState::OnRep_Downed()
{
	if (APMEPlayerCharacter* C = GetPawn<APMEPlayerCharacter>()) C->ApplyDownedState(bDowned);
}
