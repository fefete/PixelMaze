...
void APMEGameModeBase::BeginPlay()
{
    ...
    EnsureWorldLighting();
    if (const UPMEGameInstance* GI = GetGameInstance<UPMEGameInstance>()) CurrentLevel = FMath::Max(1, GI->CurrentLevel);
...
void APMEGameModeBase::StartMaze(const bool bUseNewSeed)
{
    ...
    if (PlayMode != EPMEPlayMode::SinglePlayer && GetConnectedPlayerCount() < 2) { EnterWaitingForPlayers(); return; }
    ...
    ActiveDecoys.Reset();
    if (ObjectiveManager) { ObjectiveManager->Destroy(); ObjectiveManager = nullptr; }
    ...
    if (ObjectiveManager)
    {
        ...
        ObjectiveManager->InitializeObjectives(MazeGenerator, PlayMode, CurrentLevel, MazeGenerator->GetActiveSeed(), ObjectiveCount, PickupCount);
    }
    ...
    for (FConstPlayerControllerIterator Iterator =
         ...
         ++Iterator)
    {
        if (APMEPlayerController* Controller =
            Cast<APMEPlayerController>(Iterator->Get()))
        {
            Controller->ClientNotifyMapStarted();
        }
    }
...
{
    if (!HasAuthority() || !PlayerCharacter || !MazeGameState || MazeGameState->IsWaitingForPlayers() || MazeGameState->IsRoundFinished()) return;
...
{
    if (!HasAuthority() || !PlayerCharacter || !MazeGenerator || !MazeGameState || MazeGameState->IsWaitingForPlayers() || MazeGameState->IsRoundFinished()) return;
    ...
    if (APMEPlayerController* PC = Cast<APMEPlayerController>(PlayerCharacter->GetController())) PC->ClientPlayEnemyHitSound();
    PS->RecordHit();
    ...
    const float Health = PS->GetAbilitySystemComponent()
        ? PS->GetAbilitySystemComponent()->GetNumericAttribute(UPMEAttributeSet::GetHealthAttribute()) : 0.0f;
    ...
    if (APMEPlayerController* PC = Cast<APMEPlayerController>(PlayerCharacter->GetController())) PC->ResetIgnoreMoveInput();
    PlayerCharacter->GetCharacterMovement()->StopMovementImmediately();
...
void APMEGameModeBase::FinishRound(APMEPlayerState* WinnerPlayerState, const float CompletionTime, const bool bDefeat)
{
    ...
    FString WinnerName;
    if (WinnerPlayerState) { WinnerIndex = WinnerPlayerState->GetPlayerIndex(); WinnerName = WinnerPlayerState->GetPlayerName(); }
    else if (!bDefeat && PlayMode == EPMEPlayMode::Coop) WinnerName = UPMELocalizationLibrary::GetText(TEXT("Game.Team")).ToString();
    ...
    if (!bDefeat)
    {
        for (FConstPlayerControllerIterator Iterator =
             ...
             ++Iterator)
        {
            APMEPlayerController* Controller =
                Cast<APMEPlayerController>(Iterator->Get());
            if (!Controller)
            {
                continue;
            ...
            const APMEPlayerState* LocalPlayerState =
                Controller->GetPlayerState<APMEPlayerState>();

            const bool bCompletedThisMap =
                PlayMode != EPMEPlayMode::Versus ||
                (WinnerPlayerState &&
                 LocalPlayerState == WinnerPlayerState);

            if (bCompletedThisMap)
            {
                Controller->ClientNotifyMapCompleted();
            }

            // Completing the final successful match counts as a completed
            // run for every participant. The profile stores the mode-specific
            // counter independently for future achievements.
            if (CurrentLevel >= MaximumRunLevels)
            {
                Controller->ClientNotifyRunCompleted(PlayMode);
            }
        }
    }
    ...
    {
        GetWorldTimerManager().SetTimer(TransitionTimer, this, &APMEGameModeBase::BeginUpgradeSelection, TransitionDelay, false);
    }
}
...
{
    if (!MazeGameState || MazeGameState->GetRoundPhase() == EPMERoundPhase::Escape || MazeGameState->IsRoundFinished()) return;
    MazeGameState->SetRoundPhase(EPMERoundPhase::Escape);
    if (ExitActor) ExitActor->SetExitEnabled(true);
    ApplyEscapeEnemyEscalation();
    if (MazeGenerator) EmitNoise(MazeGenerator->GetExitWorldLocation(), 16.0f, ExitActor, TEXT("Event.Noise.ExitUnlocked"));
}
...
void APMEGameModeBase::EmitNoise(const FVector& Location, const float RadiusInTiles, AActor* Source, const FName NoiseTag)
{
    if (!HasAuthority() || !MazeGenerator) return;
    float Radius = RadiusInTiles;
    if (CurrentModifier == EPMELevelModifier::Echo || CurrentModifier == EPMELevelModifier::SensitiveHearing) Radius *= 1.5f;
...
void APMEGameModeBase::SpawnDecoy(APMEPlayerCharacter* SourceCharacter)
{
    ...
    {
        Decoy->InitializeDecoy(6.0f, 0.8f, 11.0f); ActiveDecoys.Add(Decoy);
    }
}
...
void APMEGameModeBase::StunEnemiesAround(const FVector& Location, const float RadiusInTiles, const float Duration)
{
    ...
    for (APMEPatrolEnemy* Enemy : PatrolEnemies)
        if (Enemy && FVector::DistSquared2D(Enemy->GetActorLocation(), Location) <= FMath::Square(RadiusWorld)) Enemy->ApplyStun(Duration);
...
void APMEGameModeBase::TryReviveNearestTeammate(APMEPlayerCharacter* Reviver)
{
    ...
    for (FConstPlayerControllerIterator It = GetWorld()->GetPlayerControllerIterator(); It; ++It)
    {
        ...
        if (FVector::DistSquared2D(Target->GetActorLocation(), Reviver->GetActorLocation()) > FMath::Square(Radius)) continue;
        TargetPS->SetDowned(false);
        if (UAbilitySystemComponent* ASC = TargetPS->GetAbilitySystemComponent())
            ASC->SetNumericAttributeBase(UPMEAttributeSet::GetHealthAttribute(), FMath::Max(1.0f, ASC->GetNumericAttribute(UPMEAttributeSet::GetMaxHealthAttribute()) * 0.5f));
...
void APMEGameModeBase::BeginUpgradeSelection()
{
    ...
    GetWorldTimerManager().SetTimer(TransitionTimer, this, &APMEGameModeBase::ResolveUpgradeTimeout, FMath::Max(0.1f, UpgradeSelectionTimeout), false);
}
...
TArray<EPMEUpgradeType> APMEGameModeBase::BuildUpgradeChoices(const APMEPlayerState* PlayerState) const
{
    ...
    FRandomStream Stream((MazeGenerator ? MazeGenerator->GetActiveSeed() : CurrentLevel * 101) ^ ((PlayerState ? PlayerState->GetPlayerIndex() : 1) * 7919));
    TArray<EPMEUpgradeType> Result;
    while (Result.Num() < 3 && Pool.Num() > 0) { const int32 I = Stream.RandRange(0, Pool.Num() - 1); Result.Add(Pool[I]); Pool.RemoveAtSwap(I); }
    return Result;
}

void APMEGameModeBase::HandleUpgradeSelected(APMEPlayerState* PlayerState, const EPMEUpgradeType UpgradeType)
{
    if (!HasAuthority() || !MazeGameState || MazeGameState->GetRoundPhase() != EPMERoundPhase::UpgradeSelection || !PlayerState) return;
    const TWeakObjectPtr<APMEPlayerState> Key(PlayerState);
    if (PlayersWithUpgradeSelection.Contains(Key)) return;
    PlayerState->AddUpgrade(UpgradeType); PlayersWithUpgradeSelection.Add(Key);
    if (HaveAllPlayersSelectedUpgrade()) { GetWorldTimerManager().ClearTimer(TransitionTimer); GetWorldTimerManager().SetTimerForNextTick(this, &APMEGameModeBase::AdvanceToNextMaze); }
}
...
void APMEGameModeBase::ResolveUpgradeTimeout()
{
    ...
    {
        APMEPlayerState* PS = Cast<APMEPlayerState>(Base); if (!PS || PlayersWithUpgradeSelection.Contains(TWeakObjectPtr<APMEPlayerState>(PS))) continue;
        const TArray<EPMEUpgradeType> Choices = BuildUpgradeChoices(PS); if (Choices.Num() > 0) { PS->AddUpgrade(Choices[0]); PlayersWithUpgradeSelection.Add(TWeakObjectPtr<APMEPlayerState>(PS)); }
    }
    AdvanceToNextMaze();
}
...
{
    int32 Count = 0; for (APlayerState* Base : MazeGameState->PlayerArray) if (Cast<APMEPlayerState>(Base)) ++Count;
...
{
    int32 Players = 0; int32 Downed = 0;
    for (APlayerState* Base : MazeGameState->PlayerArray) if (const APMEPlayerState* PS = Cast<APMEPlayerState>(Base)) { ++Players; if (PS->IsDowned()) ++Downed; }
...
{
    for (APlayerState* Base : MazeGameState->PlayerArray) if (APMEPlayerState* PS = Cast<APMEPlayerState>(Base)) if (PS != PlayerState && !PS->IsDowned()) return PS;
    return nullptr;
}
