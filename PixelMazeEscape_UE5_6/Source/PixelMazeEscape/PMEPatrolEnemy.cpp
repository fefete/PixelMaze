...
void APMEPatrolEnemy::Tick(const float DeltaSeconds)
{
	...
	{
	case EPMEEnemyBehaviourState::Chase: TickChase(DeltaSeconds);
		break;
	case EPMEEnemyBehaviourState::ReturnToPatrol: TickReturn(DeltaSeconds);
		break;
	case EPMEEnemyBehaviourState::Investigate: TickInvestigate(DeltaSeconds);
		break;
	case EPMEEnemyBehaviourState::Patrol:
	default: TickPatrol(DeltaSeconds);
		break;
	}
}
	...
	const EPMEEnemyArchetype InArchetype)
{
	...
	{
		SightRadiusInTiles *= 0.5f;
		HearingRadiusInTiles *= 1.8f;
		PatrolMovementSpeed *= 0.9f;
	}
	else if (EnemyArchetype == EPMEEnemyArchetype::Stalker)
	{
		SightRadiusInTiles *= 1.15f;
		ChaseDistanceInTiles *= 1.35f;
		ChaseMovementSpeed *= 1.12f;
	}
...
void APMEPatrolEnemy::TickPatrol(const float DeltaSeconds)
{
	...
	const float DistanceMoved =
		MoveTowardsLocation(TargetLocation, DeltaSeconds, PatrolMovementSpeed * AlertMultiplier);
...
void APMEPatrolEnemy::HearNoise(const FVector& NoiseLocation, const float RadiusInTiles, AActor* NoiseSource,
                                const FName NoiseTag)
{
	if (!HasAuthority() || !bPatrolEnabled || !MazeGenerator || IsCrossingTransitOnlySection()) return;
	if (BehaviourState == EPMEEnemyBehaviourState::Chase || BehaviourState == EPMEEnemyBehaviourState::ReturnToPatrol ||
		BehaviourState == EPMEEnemyBehaviourState::Stunned) return;
...
void APMEPatrolEnemy::ApplyStun(const float Duration)
{
	...
	BehaviourState = EPMEEnemyBehaviourState::Stunned;
	RemainingStunTime = FMath::Max(RemainingStunTime, Duration);
	ChaseTarget.Reset();
	ActivePathPoints.Reset();
	ForceNetUpdate();
}
	...
	for (TActorIterator<APMEPlayerCharacter> It(GetWorld()); It; ++It)
	{
		...
		const float DetectionMultiplier = TargetASC
			                                  ? TargetASC->GetNumericAttribute(
				                                  UPMEAttributeSet::GetDetectionMultiplierAttribute())
			                                  : 1.0f;
